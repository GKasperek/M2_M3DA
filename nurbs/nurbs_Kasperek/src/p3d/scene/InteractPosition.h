...
class InteractPosition {
  ...
  Camera *_cameraInteract;              // contains the tool to transform 2d screen in 3d world
...
public:
  ...
  void cameraInteract(Camera *c) {_cameraInteract=c;}
  ...
  virtual void interactInsert(unsigned int ,const p3d::Vector3 &) {}
  virtual double *interactValue(unsigned int ) {return 0;}
